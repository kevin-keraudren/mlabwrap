...
def check_PATH_for_folder(folder):
    ...
    the path to try and get around capitlisation issues. """
...
def find_matlab_root():
    """ Try to find Matlab root by looking in the PATH and default installation directories"""
    if isOSX():
        base_paths = ['/Applications/']
    elif isLinux():
        base_paths = ['/usr/local/MATLAB/', '/usr/local/']
    elif isWindows():
        base_paths = [os.environ["ProgramFiles"] + 'MATLAB', os.environ["ProgramFiles(x86)"] + 'MATLAB', os.environ["ProgramFiles"], os.environ["ProgramFiles(x86)"]]
    else:
    ...
    if path_dir:
		...
		# Matlab environment variables may be set (e.g. on Windows). Therefore, find
		# the bin path and return the root directory
		try:
...
def get_matlab_output(matlab_bin):
    ...
    communicate way. This doesn't poll and assumes that program writes output and
    then exits. """
    command = [matlab_bin, '-r', 'get_version', '-nosplash', '-nodesktop']
    # On Windows we can't actually write to stdout apparently so we use the logfile
    # trick and just sit and wait until Matlab is finished loading the popup command window
    if isWindows():
...
def get_matlab_info(matlab_bin):
    ...
    matlab_version = float('{0}.{1}'.format(matlab_version[0], matlab_version[1]))
    # Slice off brackets
...
def msvc_include_dirs():
    if not 'INCLUDE' in os.environ:
        raise Exception('Unable to find Microsoft Visual Studio include directories. This is because the INCLUDE environment variable is not set. In order to install using MSVC you need to run Python from within a Microsft Visual Studio command prompt.')
    return os.environ['INCLUDE'].split(os.path.sep)

def msvc_library_dirs():
    if not 'LIB' in os.environ:
        raise Exception('Unable to find Microsoft Visual Studio include directories. This is because the LIB environment variable is not set. In order to install using MSVC you need to run Python from within a Microsft Visual Studio command prompt.')
    return os.environ['LIB'].split(os.path.sep)

def get_extension(EXTENSION_NAME, DEFINE_MACROS, MATLAB_LIBRARY_DIRS, MATLAB_LIBRARIES, CPP_LIBRARIES, MATLAB_INCLUDE_DIRS):
...
def main(args):
    ...
    if not os.path.exists(MATLAB_BINARY):
        raise Exception('Unable to find Matlab binary at {0}'.format(MATLAB_BINARY))
    ...
    #   3. Run Matlab itself and query it's 'version' command. Matlab documentation
    #      tells me that this command exists at least as far back as R14. This
    #      is the method I've chosen as it also allows me to query the 'computer'
    #      command which says which architecture Matlab is running (32 bit/64 bit).
    #      I created a short Matlab script to print the appropriate output to stdout
    MATLAB_INFO = get_matlab_info(MATLAB_BINARY)
    
    if None in MATLAB_INFO:
        raise Exception('Unable to correctly parse Matlab information. Found Matlab Version = {0}, Matlab Release = {1}, Matlab Architecture = {2}.'.format(*MATLAB_INFO))
    else:
    ...
    if isWindows():
        ...
        MATLAB_LIBRARY_DIRS = [os.path.join(MATLAB_ROOT, 'extern', 'lib', PLATFORM_DIR, 'microsoft')]
        ...
        if MATLAB_VERSION >= 7:
            ...
            MATLAB_LIBRARY_DIRS = [os.path.join(MATLAB_ROOT, 'extern', 'lib', PLATFORM_DIR)]
    ...
    setup(
            ...
            ext_modules = [
                get_extension(EXTENSION_NAME, DEFINE_MACROS, MATLAB_LIBRARY_DIRS, MATLAB_LIBRARIES, CPP_LIBRARIES, MATLAB_INCLUDE_DIRS)
            ],
            use_2to3 = True
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--matlab-root', help='The full path to the Matlab root e.g. /usr/local/MATLAB/R2013a/', default=find_matlab_root())
